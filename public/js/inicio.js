...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'VERSION_UPDATE') {
                console.log('🚨 Mensaje de actualización recibido:', event.data);
                showUpdateModal({
                    currentVersion: event.data.oldVersion,
                    serverVersion: event.data.newVersion,
                    hasUpdate: true
                });
            }
        });
    ...
    // Listener para evento de versión diferente desde ajustes
    window.addEventListener('versionMismatch', (event) => {
        console.log('🚨 Evento de versión diferente recibido:', event.detail);
        showUpdateModal({
            currentVersion: event.detail.currentVersion,
            serverVersion: event.detail.serverVersion,
            hasUpdate: true
        });
    });
});
...
function initVersionCheck() {
    // Verificar versión inmediatamente y después de 3 segundos
    checkForUpdates();
...
async function checkForUpdates() {
    ...
    if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
            ...
            messageChannel.port1.onmessage = (event) => {
                if (event.data.success && event.data.versionInfo) {
                    ...
                    if (versionInfo.hasUpdate) {
                        console.log('🚨 ¡VERSIÓN DIFERENTE DETECTADA!');
                        console.log('📱 Actual:', versionInfo.currentVersion);
                        console.log('🌐 Servidor:', versionInfo.serverVersion);
                        console.log('🔄 Mostrando modal de actualización...');
                        showUpdateModal(versionInfo);
                    } else {
                        console.log('✅ Versiones coinciden');
                        console.log('📱 Versión actual:', versionInfo.currentVersion);
                    }
                } else {
                    console.log('❌ No se pudo verificar versión:', event.data);
                }
            };
        ...
        // Fallback: verificar directamente desde la página
        try {
            const serverResponse = await fetch('/api/version?t=' + Date.now());
            if (serverResponse.ok) {
                const serverData = await serverResponse.json();
                console.log('🌐 Versión del servidor:', serverData.version);
                
                // Obtener versión actual del cache (no del meta tag actual)
                let currentVersion = 'kashly v1';
                if ('caches' in window) {
                    try {
                        ...
                        if (cachedResponse) {
                            ...
                            if (metaMatch) {
                                ...
                                console.log('⚠️ No se encontró versión en el cache');
                            }
                        } else {
                            console.log('⚠️ No hay cache de la página principal');
                        }
                    } catch (error) {
                ...
                if (serverData.version !== currentVersion) {
                    ...
                    console.log('🌐 Servidor:', serverData.version);
                    showUpdateModal({
...
function setupUpdateModal() {
    ...
    updateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            mostrarNotificacion('Actualización completada. Recargando...', 'success');
            
            // Recargar página automáticamente después de actualizar
            setTimeout(() => {
                console.log('🔄 Recargando página automáticamente...');
                console.log('✅ Cache actualizado correctamente, recargando para aplicar cambios...');
                window.location.reload(true); // true = forzar recarga sin cache
            }, 2000); // Aumentar tiempo para asegurar que se complete la actualización
        } catch (error) {
            console.error('❌ Error en actualización:', error);
            updateBtn.innerHTML = '<i class="fa-solid fa-exclamation-triangle"></i> Error';
            mostrarNotificacion('Error en la actualización', 'error');
            setTimeout(() => {
            ...
            fetch('/?t=' + Date.now())
                .then(response => {
                    if (response.ok) {
                        ...
                        resolve({ updated: ['Manual'], version: 'Actualizada' });
                    } else {
                        reject(new Error('Error en actualización manual'));
                    }
                })
                .catch(error => {
                    reject(error);
